...
def test_create_rss_feed():
    ...
    response = client.post(
        "/api/rss_feeds", json={"url": "https://example.com/rss", "category": "Tech"}
    )
...
def test_get_rss_feed_by_id():
    mock_rss_feeds_collection = MagicMock()
    mock_rss_feeds_collection.find_one.return_value = {
        "id": "feed1",
...
def test_update_rss_feed():
    ...
    response = client.put(
        "/api/rss_feeds/feed1", json={"url": "https://updated.com/rss", "category": "Updated"}
    )
    assert response.status_code == 200
...
def test_update_rss_feed_not_found():
    ...
    response = client.put("/api/rss_feeds/nonexistent", json={"url": "https://updated.com/rss"})
    assert response.status_code == 404
    assert response.json() == {"detail": "RSS Feed not found."}
